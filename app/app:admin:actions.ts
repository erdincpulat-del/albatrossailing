...
export async function listLicenses() {
  ...
  const { data, error } = await supabase
    .from("licenses")
    .select(
      "id, code, short_code, full_name, role, program, issue_date, valid_until, location, status, created_at"
    )
...
export async function upsertLicense(payload: {
  ...
  const qr_url = short_code
    ? `https://albatrossailing.com/verify?code=${encodeURIComponent(
        code
      )}&sc=${encodeURIComponent(short_code)}`
...
export async function generateCodeAndShort() {
  ...
  const { data: codeData, error: codeErr } =
    await supabase.rpc("next_license_code");
  ...
  for (let i = 0; i < 10; i++) {
    const { data: scData, error: scErr } =
      await supabase.rpc("gen_short_code");
